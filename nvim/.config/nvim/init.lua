...
do
  ...
  vim.o.relativenumber = true -- CUSTOM: enabled
...
-- CUSTOM: everything additive (options, keymaps, autocmds, PATH) lives in lua/custom/init.lua.
-- Loaded after kickstart's options/keymaps so the custom values win, and before any
-- plugin so PATH fixes (Volta/Homebrew) apply to everything plugins spawn.
require 'custom'
...
do
  ...
  require('which-key').setup {
    ...
    spec = {
      { '<leader>s', group = '[S]earch/[S]plit', mode = { 'n', 'v' } }, -- CUSTOM: renamed
  ...
  -- CUSTOM: kickstart's tokyonight block is replaced by catppuccin,
  -- installed and applied in lua/custom/plugins/catppuccin.lua
...
do
  ...
  local telescope_plugins = {
    ...
    gh 'ThePrimeagen/git-worktree.nvim', -- CUSTOM
  }
  ...
  pcall(require('telescope').load_extension, 'git_worktree') -- CUSTOM
...
do
  ...
  -- CUSTOM: Json and Yaml schemas, referenced in the servers table below
  ...
  local servers = {
    ...
    -- CUSTOM: server list diverges from kickstart (which ships only stylua/lua_ls)
    basedpyright = {
      analysis = {
    ...
    lua_ls = {
      ...
      settings = {
        Lua = {
          ...
          completion = { callSnippet = 'Replace' }, -- CUSTOM
        },
      },
    },
  }
  ...
  vim.list_extend(ensure_installed, {
    ...
    'oxfmt', -- CUSTOM: Javascript, Typescript, JSON formatter
    'hclfmt', -- CUSTOM
  })
...
do
  ...
  require('conform').setup {
    ...
    format_on_save = function(bufnr)
      -- CUSTOM: format everything on save EXCEPT these filetypes
      -- (kickstart's default is an opt-in allowlist)
    ...
    formatters = {
      -- terraform files are formatted by OpenTofu (the terraform binary is gone)
      terraform_fmt = {
        command = 'tofu',
      },
      hclfmt = {
...
do
  ...
  require('blink.cmp').setup {
    ...
    fuzzy = { implementation = 'prefer_rust_with_warning' }, -- CUSTOM: kickstart defaults to 'lua'
...
do
  ...
  -- CUSTOM: extra parsers
  vim.list_extend(parsers, { 'dockerfile', 'javascript', 'json', 'python', 'ssh_config', 'terraform', 'toml', 'typescript', 'yaml' })
...
do
  ...
  require 'custom.plugins' -- CUSTOM: enabled
  --
